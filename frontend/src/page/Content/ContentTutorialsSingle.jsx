...
const ContentTutorialsSingle = () => {
  ...
  return (
    <>
      ...
      <SectionHeader>
        {tutorial.topic().emoji} <b>{tutorial.key}</b> &nbsp; (part of <Link to={`/~cs1531/${getters.term}/content/tutorials/topic#${tutorial.topic().name}`}>{tutorial.topic().name}</Link> in {tutorial.topic().area().name})
      </SectionHeader>
